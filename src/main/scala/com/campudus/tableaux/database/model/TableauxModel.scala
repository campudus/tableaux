...
sealed trait StructureDelegateModel extends DatabaseQuery {
  ...
  def createTable(name: String, hidden: Boolean): Future[Table] = {
    structureModel.tableStruc.create(
      name,
      hidden,
      None,
      List(),
  ...
  def retrieveDependencies(
      table: Table
  ): Future[Seq[DependentColumnInformation]] = {
    structureModel.columnStruc.retrieveDependencies(table.id)
  }

  def retrieveDependentGroupColumns(
      column: ColumnType[_]
  ): Future[Seq[DependentColumnInformation]] = {
    structureModel.columnStruc.retrieveDependentGroupColumn(
      column.table.id,
      column.id
    )
  }

  def retrieveDependentLinks(
      table: Table
  ): Future[Seq[(LinkId, LinkDirection)]] = {
...
class TableauxModel(
    ...
    override implicit val requestContext: RequestContext,
  ...
  def retrieveBacklink(column: LinkColumn): Future[Option[LinkColumn]] = {
    ...
    for {
      foreignColumnIdOpt <- connection
        ...
        .map(json =>
          resultObjectToJsonArray(json).headOption.map(row =>
            row.get[ColumnId](0)
          )
        )
      ...
      backlinkColumnOpt <- foreignColumnIdOpt match {
        case Some(columnId) =>
          structureModel.columnStruc
            .retrieve(column.to.table, columnId)
            .map(res => Some(res.asInstanceOf[LinkColumn]))
        case None => Future.successful(None)
      }
    } yield backlinkColumnOpt
  }

  private def selectDependentRows(
      linkId: LinkId,
  ...
  private def retrieveDependentTableAndRowIds(
    ...
    connection
      ...
      .map(result =>
        resultObjectToJsonArray(result).map(_.getLong(0).longValue())
      )
      .map(dependentRows => (linkDirection.to, dependentRows))
  }

  def retrieveDependentRows(
      table: Table,
      rowId: RowId
    ...
    for {
      ...
      result <- {
        val futures = links.map({ case (linkId, linkDirection) =>
          retrieveDependentTableAndRowIds(linkId, linkDirection, rowId)
            .flatMap({ case (tableId, rows) =>
              for {
      ...
      val objects = result
        .groupBy({ case (dependentTable, column, _) =>
          (dependentTable, column)
        })
        .map({
          case ((groupedByTable, groupedByColumn), dependentRowInformation) =>
            (
              ...
              dependentRowInformation
                .flatMap({ case (_, _, values) =>
                  values
                })
                .distinct
            )
        })
        .filter({ case (_, _, values) =>
          values.nonEmpty
        })
        .map({ case (groupedByTable, groupedByColumn, dependentRows) =>
  ...
  def retrieveDependentCells(
    ...
    for {
      ...
      result <- Future.sequence(
        links.map({ case (linkId, linkDirection) =>
          retrieveDependentTableAndRowIds(linkId, linkDirection, rowId)
            .flatMap({ case (dependentTableId, dependentRows) =>
              for {
  ...
  def createRows(
      table: Table,
      rows: Seq[Seq[(ColumnId, Any)]]
  ): Future[RowSeq] = {
    for {
      allColumns <- retrieveColumns(table)
      columns = roleModel.filterDomainObjects(
        ScopeColumn,
        allColumns,
        ComparisonObjects(table),
      ...
      rows <- rows.foldLeft(Future.successful(Vector[Row]())) {
        (futureRows, row) => // replace ColumnId with ColumnType
          // TODO fail nice if columnid doesn't exist
          {
            val columnValuePairs = row.map { case (columnId, value) =>
              (columns.find(_.id == columnId).get, value)
            }
            ...
            futureRows.flatMap { rows =>
              for {
                ...
                _ <- createHistoryModel.createRow(table, rowId)
                _ <- createHistoryModel.createCells(
                  table,
                  rowId,
                  columnValuePairs
                )
  ...
  def addCellAnnotation(
    ...
    for {
      (uuid, mergedLangtags, createdAt) <- updateRowModel
        .addOrMergeCellAnnotation(
          column,
          rowId,
          langtags,
          annotationType,
      ...
      _ <- createHistoryModel.addCellAnnotation(
        column,
        rowId,
        uuid,
        langtags,
  ...
  def deleteCellAnnotation(
    ...
    for {
      annotationOpt <- retrieveRowModel.retrieveAnnotation(
        column.table.id,
        rowId,
        column,
        uuid
      ...
      _ <- annotationOpt match {
        case Some(annotation) =>
          createHistoryModel.removeCellAnnotation(
            column,
            rowId,
            uuid,
            annotation
  ...
  def updateRowAnnotations(
    ...
    for {
      ...
      _ <- createHistoryModel.updateRowsAnnotation(
        table.id,
        Seq(rowId),
        finalFlag
      )
  ...
  def updateRowsAnnotations(
    ...
    for {
      ...
      rowIds <- retrieveRows(table, Pagination(None, None)).map(
        _.rows.map(_.id)
      )
  ...
  def retrieveTableWithCellAnnotations(
      table: Table
  ): Future[TableWithCellAnnotations] = {
    retrieveTablesWithCellAnnotations(Seq(table)).map({ annotations =>
      annotations.headOption.getOrElse(
        TableWithCellAnnotations(table, Map.empty)
      )
    })
  }

  def retrieveTablesWithCellAnnotations(
      tables: Seq[Table]
  ): Future[Seq[TableWithCellAnnotations]] = {
    retrieveRowModel.retrieveTablesWithCellAnnotations(tables)
  }

  def retrieveTablesWithCellAnnotationCount(
      tables: Seq[Table]
  ): Future[Seq[TableWithCellAnnotationCount]] = {
    val tableIds = tables.map({ case Table(id, _, _, _, _, _, _, _) => id })

    for {
      annotationCountMap <- retrieveRowModel.retrieveCellAnnotationCount(
        tableIds
      )
      totalSizeMap <- Future.sequence(
        tables.map(table => retrieveTotalSize(table).map((table, _)))
      )
    } yield {
      totalSizeMap.map({ case (table, count) =>
  ...
  def deleteLink(
    ...
    for {
      ...
      _ <- roleModel.checkAuthorization(
        EditCellValue,
        ScopeColumn,
        ComparisonObjects(table, column)
      )

      _ <- column match {
        case linkColumn: LinkColumn => {
          for {
            _ <- createHistoryModel.createCellsInit(
              table,
              rowId,
              Seq((column, Seq(toId)))
            )
            _ <- updateRowModel.deleteLink(
              table,
              linkColumn,
              rowId,
              toId,
        ...
        case _ =>
          Future.failed(WrongColumnKindException(column, classOf[LinkColumn]))
      }
        ...
        case linkColumn: LinkColumn => {
          for {
            _ <- createHistoryModel.createCellsInit(
              table,
              rowId,
              Seq((linkColumn, Seq(rowId)))
            )
            _ <- updateRowModel.updateLinkOrder(
              table,
              linkColumn,
              rowId,
              toId,
  ...
  private def checkValueTypeForColumn[A](
      column: ColumnType[_],
      value: A
  ): Future[Unit] = {
    (column match {
  ...
  private def updateOrReplaceValue[A](
    ...
    for {
      _ <- checkForSettingsTable(
        table,
        columnId,
        "can't update key cell of a settings table"
      )
      ...
      _ <-
        if (replace && column.languageType == MultiLanguage) {
          ...
          val enrichedValue =
            roleModel.generateLangtagCheckValue(table, valueJson.copy())
          roleModel.checkAuthorization(
            EditCellValue,
            ScopeColumn,
            ComparisonObjects(table, column, enrichedValue)
          )
        } else {
          roleModel.checkAuthorization(
            EditCellValue,
            ScopeColumn,
            ComparisonObjects(table, column, value)
          )
        }
      ...
      _ <-
        if (replace) {
          for {
            _ <- createHistoryModel.clearBackLinksWhichWillBeDeleted(
              table,
              rowId,
              Seq((column, value))
            )
            _ <- updateRowModel.clearRowWithValues(
              table,
              rowId,
              Seq((column, value)),
              deleteRow
  ...
  def clearCellValue(
    ...
    for {
      _ <- checkForSettingsTable(
        table,
        columnId,
        "can't clear key cell of a settings table"
      )
  ...
  private def checkForSettingsTable[A](
      table: Table,
      columnId: ColumnId,
  ...
  private def checkForDuplicateKey[A](
      table: Table,
      keyColumn: ColumnType[_],
      keyName: Option[Any]
  ) = {
      ...
      case _ =>
        Future.failed(
          InvalidRequestException(
            "Key must not be empty and a string in settings table"
          )
        )
    }
  }

  def invalidateCellAndDependentColumns(
      column: ColumnType[_],
      rowIds: Seq[RowId]
  ): Future[Seq[Unit]] = {
    Future.sequence(
  ...
  def retrieveAllStatusColumnsForTable(
      table: Table
    ...
    for {
      ...
      .map(column => column.asInstanceOf[StatusColumn])
  }

  def maybeInvalidateStatusCells(
      column: ColumnType[_],
      rowId: RowId
  ): Future[Unit] = {
    for {
      ...
      _ = statusColumns.foreach((statusColumn: StatusColumn) => {
        if (statusColumn.columns.map(column => column.id).contains(column.id)) {
          CacheClient(this.connection)
            .invalidateCellValue(statusColumn.table.id, statusColumn.id, rowId)
        }
      })
    } yield ()
  }
  ...
  def invalidateCellAndDependentColumns(
    ...
    def invalidateColumn: (TableId, ColumnId) => Future[_] = CacheClient(
      this.connection
    ).invalidateColumn

    for {
      // invalidate the cell itself
      _ <- CacheClient(this.connection)
        .invalidateCellValue(column.table.id, column.id, rowId)
      ...
      _ <-
        if (column.identifier) {
          CacheClient(this.connection)
            .invalidateCellValue(column.table.id, 0, rowId)
        } else {
          Future.successful(())
        }
      ...
      _ <-
        if (column.columnInformation.groupColumnIds.nonEmpty) {
          Future.sequence(
            column.columnInformation.groupColumnIds.map(
              invalidateColumn(column.table.id, _)
            )
          )
        } else {
          Future.successful(())
      ...
      _ <- Future.sequence(dependentColumns.map({
        ...
        case DependentColumnInformation(
              tableId,
              columnId,
              _,
              true,
          ...
          invalidateLinkColumn
            .zip(invalidateConcatColumn)
            .zip(invalidateGroupColumns)
  ...
  def retrieveCellAnnotations(
    ...
    for {
      ...
      (_, cellLevelAnnotations) <- retrieveRowModel.retrieveAnnotations(
        column.table.id,
        rowId,
        Seq(column)
      )
  ...
  private def retrieveCell(
    ...
    for {
      valueCache <- CacheClient(this.connection)
        .retrieveCellValue(column.table.id, column.id, rowId)
          ...
          for {
            rowSeq <- column match {
              case _: AttachmentColumn =>
                ...
                for {
                  (rowLevelAnnotations, cellLevelAnnotations) <-
                    retrieveRowModel
                      .retrieveAnnotations(column.table.id, rowId, Seq(column))
                  attachments <- attachmentModel.retrieveAll(
                    column.table.id,
                    column.id,
                    rowId
                  )
              ...
              case _ =>
                for {
                  rawRows <- retrieveRowModel.retrieve(
                    column.table.id,
                    rowId,
                    columns
                  )
            ...
            CacheClient(this.connection).setCellValue(
              column.table.id,
              column.id,
              rowId,
              value
  ...
  def retrieveForeignRows(
    ...
    for {
      ...
      representingColumns <- retrieveColumns(
        foreignTable,
        isInternalCall = true
      )
        .map({ foreignColumns =>
  ...
  private def copyFirstColumnOfRowsSeq(rowsSeq: RowSeq): RowSeq = {
    rowsSeq.copy(rows =
      rowsSeq.rows.map(row => row.copy(values = row.values.take(1)))
    )
  }
  ...
  private def mapRawRows(
    ...
    def fetchConcatValuesForLinkedRows(
      ...
      linkedRows.asScala
        ...
        .foldLeft(Future.successful(List.empty[JsonObject])) {
          case (futureList, linkedRow: JsonObject) =>
            ...
            for {
              ...
              Json.obj(
                "id" -> rowId,
                "value" -> DomainObject.compatibilityGet(cell.value)
              )
            )
        }
    }
    ...
    def calcStatusValue(
        rules: JsonArray,
        columnsWithValues: Map[ColumnId, (ColumnType[_], Any)]
    ): Seq[Boolean] = {
      ...
      def calcValue(condition: JsonObject): Boolean = {
        ...
        val compositionFunction = condition.getString("composition") match {
          case "OR" =>
            (acc: Boolean, value: Boolean) => acc || value
          case "AND" =>
            (acc: Boolean, value: Boolean) => acc && value
        }
          ...
          .map(value => {
            if (value.containsKey("values")) {
              ...
              val columnId: ColumnId =
                value.getLong("column").asInstanceOf[ColumnId]
    ...
    rawRows.foldLeft(Future.successful(List.empty[Row])) {
      case (
            rawRowsFuture,
            RawRow(rowId, rowLevelFlags, cellLevelFlags, rawValues)
          ) =>
        for {
          ...
          columnsWithFetchedValues <- Future.sequence(
            columns
              .zip(rawValues)
              .map({
                case (c: LinkColumn, array: JsonArray)
                    if c.to.isInstanceOf[ConcatenateColumn] =>
                  // Fetch linked values of each linked row
                  fetchConcatValuesForLinkedRows(
                    c.to.asInstanceOf[ConcatenateColumn],
                    array
                  )
                ...
                case (c: StatusColumn, value) =>
                  for {
                    dependentColumnValues <- fetchValuesForStatusColumn(
                      c.asInstanceOf[ConcatenateColumn],
                      rowId
                    )
                    statusValue = calcStatusValue(
                      c.rules,
          ...
          columnsWithPostProcessedValues = columnsWithFetchedValues.map({
            ...
            case (c: ConcatenateColumn, _) =>
              ...
              columnsWithFetchedValues
                .filter({ case (column: ColumnType[_], _) =>
                  c.columns.exists(_.id == column.id)
                })
                .map({ case (_, value) =>
                  value
                })
          ...
          Row(
            ...
            rowLevelFlags,
            cellLevelFlags,
            columnsWithPostProcessedValues
          )
        )
    }
  }

  def retrieveColumnValues(
      table: Table,
      columnId: ColumnId,
    ...
    for {
      shortTextColumn <- retrieveColumn(table, columnId).flatMap({
        case shortTextColumn: ShortTextColumn =>
          Future.successful(shortTextColumn)
        case column =>
          Future.failed(
            WrongColumnKindException(column, classOf[ShortTextColumn])
          )
      })
      ...
      _ <- roleModel.checkAuthorization(
        ViewCellValue,
        ScopeColumn,
        ComparisonObjects(table, shortTextColumn)
      )
      values <- retrieveRowModel.retrieveColumnValues(
        shortTextColumn,
        langtagOpt
      )
    } yield values
  ...
  def retrieveCellHistory(
    ...
    for {
      ...
      cellHistorySeq <- retrieveHistoryModel.retrieveCell(
        table,
        column,
        rowId,
        langtagOpt,
  ...
  def retrieveRowHistory(
    ...
    for {
      columns <- retrieveColumns(table)
      cellHistorySeq <- retrieveHistoryModel.retrieveRow(
        table,
        rowId,
        langtagOpt,
        typeOpt
      )
      filteredCellHistorySeq = filterCellHistoriesForColumns(
        cellHistorySeq,
        columns
      )
  ...
  def retrieveTableHistory(
    ...
    for {
      columns <- retrieveColumns(table)
      cellHistorySeq <- retrieveHistoryModel.retrieveTable(
        table,
        langtagOpt,
        typeOpt
      )
  ...
  private def filterCellHistoriesForColumns(
      cellHistorySeq: Seq[History],
      columns: Seq[ColumnType[_]]
  ) = {
    cellHistorySeq.filter(history => {
      val columnIds: Seq[ColumnId] = columns.map(_.id)
      history.columnIdOpt.forall(historyColumnId =>
        columnIds.contains(historyColumnId)
      )
    })
  }

  private def checkColumnTypeForLangtag[A](
      column: ColumnType[_],
      langtagOpt: Option[String]
  ): Future[Unit] = {
