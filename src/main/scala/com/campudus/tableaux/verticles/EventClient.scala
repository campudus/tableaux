...
class EventClient(val vertx: Vertx) extends VertxAccess {
  ...
  val options = DeliveryOptions().setSendTimeout(CacheVerticle.cacheTimeoutMillis)
  ...
  private def sendMessage(address: String, jsonObj: JsonObject = Json.obj()): Future[Unit] = {
     ...
     */
    eventBus.request[String](address, jsonObj).asScala.map(_ => {}).recover { case _: Throwable => }
  }
  ...
  def retrieveThumbnailPath(
    ...
    eventBus
      .request[String](ADDRESS_THUMBNAIL_RETRIEVE, message, thumbnailOptions).asScala.map(message =>
        Path(message.body())
      )
  }
  ...
  def validateJson(key: String, json: JsonObject): Future[Unit] = {
    ...
    eventBus
      .request[String](ADDRESS_JSON_SCHEMA_VALIDATE, message)
      .asScala
      .map(_ => {})
  }
  ...
  def validateJson(key: String, json: JsonArray): Future[Unit] = {
    ...
    eventBus.request[String](ADDRESS_JSON_SCHEMA_VALIDATE, message).asScala.map(_ => {})
  }

  def registerSchema(schemaWithKey: JsonObject): Future[Unit] = {
    eventBus.request[String](ADDRESS_JSON_SCHEMA_REGISTER, schemaWithKey).asScala.map(_ => {})
  }
  ...
  def setCellValue(tableId: TableId, columnId: ColumnId, rowId: RowId, value: Any): Future[_] = {
    ...
    eventBus.request(ADDRESS_SET_CELL, obj, options).asScala
  }
  ...
  def retrieveCellValue(tableId: TableId, columnId: ColumnId, rowId: RowId): Future[Option[Any]] = {
    ...
    eventBus
      .request[JsonObject](ADDRESS_RETRIEVE_CELL, obj, options)
      .asScala
      .map({
  ...
  def setRowPermissions(
    ...
    eventBus.request(ADDRESS_SET_ROW_PERMISSIONS, obj, options).asScala
  }
  ...
  def retrieveRowPermissions(tableId: TableId, rowId: RowId): Future[Option[RowPermissions]] = {
    ...
    eventBus
      .request[JsonObject](ADDRESS_RETRIEVE_ROW_PERMISSIONS, obj, options).asScala
      .map(value => {
        value match {
  ...
  def setRowLevelAnnotations(
    ...
    eventBus
      .request(ADDRESS_SET_ROW_LEVEL_ANNOTATIONS, obj, options).asScala
  }
  ...
  def retrieveRowLevelAnnotations(tableId: TableId, rowId: RowId): Future[Option[RowLevelAnnotations]] = {
    ...
    eventBus
      .request[JsonObject](ADDRESS_RETRIEVE_ROW_LEVEL_ANNOTATIONS, obj, options).asScala
      .map(value => {
        value match {
          case v if v.body().containsKey("value") => {
