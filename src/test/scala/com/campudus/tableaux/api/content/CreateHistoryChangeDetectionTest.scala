...
class CreateHistoryChangeDetectionTest extends LinkTestBase with TestHelper {
  ...
  @Test
  def changeSimpleValueTwice_changeACellOnlyOnce(implicit c: TestContext): Unit = okTest {
  ...
  @Test
  def changeSimpleValueTwice_withForceHistory(implicit c: TestContext): Unit = okTest {
    val newValue = Json.obj("value" -> "a fix value")
    ...
    for {
      ...
      _ <- sendRequest("POST", "/tables/1/rows")
      _ <- sendRequest("POST", "/tables/1/columns/1/rows/1", newValue)
      _ <- sendRequest("POST", "/tables/1/columns/1/rows/1?forceHistory=true", newValue)
      rows <- sendRequest("GET", "/tables/1/columns/1/rows/1/history?historyType=cell").map(toRowsArray)
    } yield {
  ...
  @Test
  def changeMultilanguageValueTwice_text(implicit c: TestContext): Unit = okTest {
  ...
  @Test
  def changeMultilanguageValueTwice_boolean(implicit c: TestContext): Unit = okTest {
  ...
  @Test
  def changeMultilanguageValueTwice_numeric(implicit c: TestContext): Unit = okTest {
  ...
  @Test
  def changeMultilanguageValueTwice_datetime(implicit c: TestContext): Unit = okTest {
  ...
  @Test
  def changeMultilanguageValueTwice_currency(implicit c: TestContext): Unit = okTest {
  ...
  @Test
  def changeLinkValueTwice_withForceHistory(implicit c: TestContext): Unit = okTest {
    val twoLinks = Json.obj("value" -> Json.obj("values" -> Json.arr(1, 2)))

    for {
      linkColumnId <- setupTwoTablesWithEmptyLinks()
      _ <- sendRequest("PUT", s"/tables/1/columns/$linkColumnId/rows/1", twoLinks)
      _ <- sendRequest("PUT", s"/tables/1/columns/$linkColumnId/rows/1?forceHistory=true", twoLinks)
      rows <- sendRequest("GET", s"/tables/1/columns/$linkColumnId/rows/1/history?historyType=cell").map(toRowsArray)
    } yield {
      assertEquals(2, rows.size())
    }
  ...
  @Test
  def changeLink_addTwoLinksAtOnce(implicit c: TestContext): Unit = okTest {
