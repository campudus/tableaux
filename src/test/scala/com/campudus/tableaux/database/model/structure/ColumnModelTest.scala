...
class ColumnModelTest {
  ...
  @Before
  def setup(): Unit = {
    val sc1 = CreateSimpleColumn("c1", null, null, LanguageNeutral, identifier = false, Nil)
    val sc2 = CreateSimpleColumn("c2", null, null, LanguageNeutral, identifier = false, Nil)
...
//             | [1, 2]  | "{{2}} mm × {{1}} mm" | [x]     |
  ...
  def isColumnGroupMatchingToFormatPattern_oneHigherColumnId_patternWithHigherWildcard_isValid(): Unit = {
    val sc42 =
      CreateSimpleColumn("c42", null, null, LanguageNeutral, identifier = false, List())
    val testTable = Table(1, "table", hidden = false, null, null, null, null)
